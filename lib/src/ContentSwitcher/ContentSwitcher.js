...
export function FjContentSwitcher({options, active, size, disabled=false}) {
  ...
  for (let option of options) {
    buttons.push(<button
      ...
      key={option.value}
      className={
        active[0] === option.value ? 'content-switcher__button content-switcher__button--active' : 'content-switcher__button'
      }
      style={disabled ? {backgroundColor: theme.lightGrayLighter, color: theme.gray, borderColor: theme.gray, cursor: 'default'} : null }
      onClick={
        () => { if (!disabled) {
