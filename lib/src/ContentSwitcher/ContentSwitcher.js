...
export function FjContentSwitcher({options, active, size, disabled = false, allowMultiple = false, buttonsPerRow, requireOne = true}) {
  ...
  options.forEach(option => buttons.push(buttonElement(option)));
  
  if (buttonsPerRow) {
    while (buttons.length > 0) {
      elements.push(<ContentSwitcher>{buttons.splice(0, buttonsPerRow)}</ContentSwitcher>);
    }
  } else {
    elements.push(<ContentSwitcher>{buttons}</ContentSwitcher>);
  }
